...
def get_save_folder(model, date, label, config):
    # For general experiments
    folder_path = os.path.join(
        "results", model, "{}_{}".format(date, label)
    )
...
def save_results(results, label, model, date, run_id, seed, args):
    del results["config"]["device"]

    folder_path = get_save_folder(model, date, label, args)
